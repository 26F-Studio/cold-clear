...
impl Evaluator for Standard {
    ...
    fn evaluate(
        ...
        match lock.perfect_clear {
            ...
            ClearKind::Half => {
                acc_eval += self.perfect_clear * 4 / (8 + board.pc_combo as i32 * 2).clamp(8, 16)
            }
            _ => {}
        }
        if self.stack_pc_damage || lock.perfect_clear != ClearKind::All {
            // half?
            if lock.placement_kind.is_clear() && lock.b2b >= 50 {
                // b3b?
                if lock.b2b > 800 {
            ...
            if let Some(combo) = lock.combo {
                acc_eval +=
                    self.combo_garbage * (lock.garbage_sent as i32 - lock.no_combo_sent as i32);
            }
            use B2BKind::*;
            use TspinStatus::*;
            match (
                lock.placement_kind.cleared,
                lock.placement_kind.tspin,
                lock.placement_kind.b2b,
            ) {
                (0, _, _) => {}
                (1, None, _) => {
